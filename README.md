...
- **Element location declaration** – for every image pattern the user states how the element behaves:
  *can appear anywhere on screen*, *moves within a selected area*, or *is always in the same place*.
  The engine derives the searched screen region from that declaration, which is what makes lookups fast
  (a pinned element is found in ~11 ms instead of ~460 ms).
